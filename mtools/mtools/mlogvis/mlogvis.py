    ...
    def __init__(self):
        ...
        self.argparser.add_argument('--line-max', action='store',
                                    ...
                                    help=('max count of datapoints at which '
                                          ...
                                          'printed any more.'))
        self.argparser.add_argument('--host', '-H', action='store',
                                    default=None,
                                    help=('123'))
        self.argparser.add_argument('--port', '-p', action='store',
                                    default=None,
                                    help=('123'))
        self.argparser.add_argument('--user', '-u', action='store',
                                    default=None,
                                    help=('123'))
        self.argparser.add_argument('--pwd', '-P', action='store',
                                    default=None,
                                    help=('123'))
        self.argparser.add_argument('--db', '-d', action='store',
                                    default=None,
                                    help=('123'))
...
def kpi_monitor(self,str_doc):
    ...
    print dict_list
    insert_stat(dict_list,self)

def insert_stat(items,self):
    db = MySQLdb.connect(self.args['host'], self.args['user'], self.args['pwd'], self.args['db'], charset='utf8',port=int(self.args['port']))
    cursor = db.cursor()
    ...
    for item in items:
        ...
        namespace = ""
        operation = ""
        if item.has_key("numYields"):
            numYields = item["numYields"]

        if item.has_key("namespace"):
            namespace = item["namespace"]

        if item.has_key("operation"):
            operation = item["operation"]

        sql = "insert into mongo_vis(thread,numYields,namespace,monitor_time,duration,operation,id,line_str) \
                    values ('%s','%d','%s','%s','%d','%s','%d','%s')" % \
              (item["thread"],numYields,namespace,item["datetime"][:-6],item["duration"],operation,item["_id"],line_str)
